const INV_API_URL = 'https://script.google.com/macros/s/AKfycbwuxND1GoxhhH-UUnvFBwVO8kOE49cTLiRbklIvZTmV3s_Fbjj997Ls68fMW17TXi41/exec';
...
document.querySelectorAll('#pills-tab button').forEach(btn => {
    btn.addEventListener('click', function(event) {
        ...
        document.querySelectorAll('#module-invoice .tab-pane').forEach(pane => {
...
async function loadBillingData(isLogin = false, forceSync = false) {
    if (!forceSync && rawBillingData.length > 0) {
        applyFilterAudit(); applyFilterReady(); return;
    }
    ...
    try {
        ...
        const { data: allBkgData } = await supabaseClient.from('plan_data').select('booking');
        bkgTotalsGlobal = {};
        if (allBkgData) {
            allBkgData.forEach(r => {
                if (r.booking) bkgTotalsGlobal[r.booking] = (bkgTotalsGlobal[r.booking] || 0) + 1;
            });
        }
        ...
        (data || []).forEach(row => {
            ...
            let ext1 = parseFloat(r[20]) || 0; let ext2 = parseFloat(r[21]) || 0; let ext3 = parseFloat(r[22]) || 0;
            let adv1 = parseFloat(r[18]) || 0; let adv2 = parseFloat(r[19]) || 0;
            let adv3 = parseFloat(r[24]) || 0; let adv4 = parseFloat(r[25]) || 0; let adv5 = parseFloat(r[26]) || 0; let adv6 = parseFloat(r[27]) || 0;
            let ext_v1 = parseFloat(r[29]) || 0; let ext_v2 = parseFloat(r[31]) || 0;
        ...
        updateInvDropdowns(rawBillingData);
        applyFilterAudit(); applyFilterReady();

        const { data: compList } = await supabaseClient.from('customer').select('full_name, short_name');
        if (compList) {
            let cHtml = '';
            let names = [...new Set(compList.flatMap(c => [c.full_name, c.short_name]).filter(String))];
            names.forEach(c => cHtml += `<option value="${c}">`);
            let mList = document.getElementById('modalCompanyList');
            if (mList) mList.innerHTML = cHtml;
        }
    ...
    for (let bkg of bkgKeys) {
        ...
        html += `
            <tr class="parent-row bg-white" onclick="toggleDetailA('${safeBkg}')">
                ...
                <td class="text-end fw-bold text-dark pe-4" id="pa-tot-${safeBkg}">฿${group.sumTot.toLocaleString(undefined, {minimumFractionDigits:2})}</td>
            </tr>
        `;
                            ...
                            <div class="row g-3">
                               <div class="col-md-5">
                                  <div class="bg-white border rounded-4 p-3 shadow-sm h-100">
                                      ...
                                      <div class="row g-2">
                                          <div class="col-6"><label class="small text-muted mb-1 fw-medium">ราคาเที่ยว</label><input type="number" class="form-control form-control-sm text-end inp-inc-a in-p" data-name="ราคาเที่ยว" data-orig="${parseFloat(r[17])||0}" value="${parseFloat(r[17])||0}" oninput="updateCalcA(this, '${safeBkg}')"></div>
                                          <div class="col-6"><label class="small text-muted mb-1 fw-medium">ต่อระยะ</label><input type="number" class="form-control form-control-sm text-end inp-inc-a in-ext1" data-name="ต่อระยะ" data-orig="${parseFloat(r[20])||0}" value="${parseFloat(r[20])||0}" oninput="updateCalcA(this, '${safeBkg}')"></div>
                                          <div class="col-6"><label class="small text-muted mb-1 fw-medium">ค้างหาง</label><input type="number" class="form-control form-control-sm text-end inp-inc-a in-ext2" data-name="ค้างหาง" data-orig="${parseFloat(r[21])||0}" value="${parseFloat(r[21])||0}" oninput="updateCalcA(this, '${safeBkg}')"></div>
                                          <div class="col-6"><label class="small text-muted mb-1 fw-medium">เสียเวลา</label><input type="number" class="form-control form-control-sm text-end inp-inc-a in-ext3" data-name="เสียเวลา" data-orig="${parseFloat(r[22])||0}" value="${parseFloat(r[22])||0}" oninput="updateCalcA(this, '${safeBkg}')"></div>
                                      </div>
                                  </div>
                               </div>
                               <div class="col-md-7">
                                  <div class="bg-white border rounded-4 p-3 shadow-sm h-100">
                                      ...
                                      <div class="row g-2">
                                          <div class="col-4 col-sm-3"><label class="small text-muted mb-1 fw-medium">รับตู้</label><input type="number" class="form-control form-control-sm text-end inp-adv-a in-adv1" data-name="รับตู้" data-orig="${parseFloat(r[18])||0}" value="${parseFloat(r[18])||0}" oninput="updateCalcA(this, '${safeBkg}')"></div>
                                          <div class="col-4 col-sm-3"><label class="small text-muted mb-1 fw-medium">คืนตู้</label><input type="number" class="form-control form-control-sm text-end inp-adv-a in-adv2" data-name="คืนตู้" data-orig="${parseFloat(r[19])||0}" value="${parseFloat(r[19])||0}" oninput="updateCalcA(this, '${safeBkg}')"></div>
                                          <div class="col-4 col-sm-3"><label class="small text-muted mb-1 fw-medium">ผ่านท่า</label><input type="number" class="form-control form-control-sm text-end inp-adv-a in-adv3" data-name="ผ่านท่า" data-orig="${parseFloat(r[24])||0}" value="${parseFloat(r[24])||0}" oninput="updateCalcA(this, '${safeBkg}')"></div>
                                          <div class="col-4 col-sm-3"><label class="small text-muted mb-1 fw-medium">ซ่อมตู้</label><input type="number" class="form-control form-control-sm text-end inp-adv-a in-adv4" data-name="ซ่อมตู้" data-orig="${parseFloat(r[25])||0}" value="${parseFloat(r[25])||0}" oninput="updateCalcA(this, '${safeBkg}')"></div>
                                          <div class="col-4 col-sm-3"><label class="small text-muted mb-1 fw-medium">ล้างตู้</label><input type="number" class="form-control form-control-sm text-end inp-adv-a in-adv5" data-name="ล้างตู้" data-orig="${parseFloat(r[26])||0}" value="${parseFloat(r[26])||0}" oninput="updateCalcA(this, '${safeBkg}')"></div>
                                          <div class="col-4 col-sm-3"><label class="small text-primary mb-1 fw-bold">ค่าชอ</label><input type="number" class="form-control form-control-sm text-end border-primary bg-primary bg-opacity-10 inp-adv-a in-adv6" data-name="ค่าชอ" data-orig="${parseFloat(r[27])||0}" value="${parseFloat(r[27])||0}" oninput="updateCalcA(this, '${safeBkg}')"></div>
                                          <div class="col-6 col-sm-3"><label class="small text-muted mb-1 fw-medium">ชื่อยอดอื่น 1</label><input type="text" class="form-control form-control-sm in-extn1" data-name="ชื่อยอดอื่น 1" data-orig="${r[28]||''}" value="${r[28]||''}" oninput="updateCalcA(this, '${safeBkg}')"></div>
                                          <div class="col-6 col-sm-3"><label class="small text-muted mb-1 fw-medium">ยอดเงิน 1</label><input type="number" class="form-control form-control-sm text-end inp-adv-a in-extv1" data-name="ยอดเงิน 1" data-orig="${parseFloat(r[29])||0}" value="${parseFloat(r[29])||0}" oninput="updateCalcA(this, '${safeBkg}')"></div>
                                      </div>
                                  </div>
                               </div>
                            </div>
...
function updateCalcA(inputElem, safeBkg) {
    ...
    if (origVal !== currentVal) inputElem.classList.add('bg-warning', 'bg-opacity-10', 'border-warning');
    else inputElem.classList.remove('bg-warning', 'bg-opacity-10', 'border-warning');
...
function calcSummaryA() {
    ...
    document.querySelectorAll('.bkg-check-a').forEach(bkgCb => {
        ...
        if (cChild === 0) { bkgCb.checked = false; bkgCb.indeterminate = false; } 
        else if (cChild === tChild) { bkgCb.checked = true; bkgCb.indeterminate = false; } 
        else { bkgCb.checked = false; bkgCb.indeterminate = true; }
    });
}
...
async function saveAuditBulk() {
    ...
    checkboxes.forEach(cb => {
        ...
        c.querySelectorAll('.inp-inc-a, .inp-adv-a, .in-extn1, .in-extn2').forEach(inp => {
            ...
            if (orig !== curr) {
                if (inp.type === 'number') editLogs.push(`${inp.getAttribute('data-name')} ${orig.toLocaleString()}->${curr.toLocaleString()}`);
                else editLogs.push(`${inp.getAttribute('data-name')} เปลี่ยนเป็น ${curr}`);
            }
        });
        ...
        payloadData.push({
            ...
            receive: (parseFloat(c.querySelector('.in-adv1').value) || 0).toString(),
            retrun: (parseFloat(c.querySelector('.in-adv2').value) || 0).toString(),
            extender: parseFloat(c.querySelector('.in-ext1').value) || 0,
            tail_drop: parseFloat(c.querySelector('.in-ext2').value) || 0,
            lose_time: parseFloat(c.querySelector('.in-ext3').value) || 0,
            terminal_charge: parseFloat(c.querySelector('.in-adv3').value) || 0,
            repair: (parseFloat(c.querySelector('.in-adv4').value) || 0).toString(),
            cleaning: parseFloat(c.querySelector('.in-adv5').value) || 0,
            shore: parseFloat(c.querySelector('.in-adv6').value) || 0,
    ...
    Swal.fire({
        title: 'ยืนยันการอนุมัติ?', text: `คุณกำลังส่ง ${payloadData.length} ตู้ไปยังหน้า "เตรียมวางบิล"`, icon: 'question',
        showCancelButton: true, confirmButtonColor: '#4f46e5', confirmButtonText: 'ยืนยัน', cancelButtonText: 'ยกเลิก'
    }).then(async result => {
...
function renderReadyTab() {
    ...
    filteredReady.forEach(obj => {
        ...
        groupedReadyGlobal[bkg].sumInc += obj.incTot; groupedReadyGlobal[bkg].sumAdv += obj.advTot; groupedReadyGlobal[bkg].sumTot += obj.grandTot;
    });
                    ...
                    <div class="bg-white m-2 p-3 rounded-4 shadow-sm border">
                        <table class="table table-borderless table-sm m-0" style="font-size: 13px;">
                            <thead class="text-muted border-bottom">
                                <tr><th>ตู้คอนเทนเนอร์</th><th>ทะเบียนรถ</th><th class="text-end">รายได้</th><th class="text-end">สำรองจ่าย</th><th class="text-end">สุทธิ</th></tr>
                            </thead>
...
function calcSummaryR() {
    ...
    document.querySelectorAll('.bkg-check-r:checked').forEach(cb => {
        ...
        if (group) { sInc += group.sumInc; sAdv += group.sumAdv; sTot += group.sumTot; countBkg++; }
    });
...
function revertBooking(bkg) {
    Swal.fire({
        title: 'ส่งกลับหน้าตรวจสอบ?', html: `ต้องการส่ง Booking: <b class="text-warning">${bkg}</b> กลับไปหน้าตรวจสอบยอดใหม่หรือไม่`, icon: 'warning',
        showCancelButton: true, confirmButtonColor: '#f59e0b', confirmButtonText: 'ยืนยัน', cancelButtonText: 'ยกเลิก'
    }).then(async res => {
        if (res.isConfirmed) {
...
async function confirmGeneratePDF() {
    ...
    try {
        ...
        let custInfoToSend = {
            officialName: custData ? custData.full_name : finalBillTo,
            address: custData && custData.address ? custData.address : "",
            taxId: custData && custData.tax_id ? String(custData.tax_id) : "",
            creditDays: custData && custData.credit_days ? parseInt(custData.credit_days) : 0
        };

        let yy = (new Date(invDate).getFullYear() + 543).toString().slice(-2);
        let mm = String(new Date(invDate).getMonth() + 1).padStart(2, '0');
        let prefix = "INV" + yy + mm;
        let finalInvNo = customInvNo;
        ...
        if (!finalInvNo) {
            ...
            let nextSeq = 1;
            if (lastInv && lastInv.length > 0 && lastInv[0].invoice_no) {
                let lastSeqStr = lastInv[0].invoice_no.replace(prefix, '').split('-')[0];
        ...
        if (res.success) {
            ...
            const { error: planError } = await supabaseClient.from('plan_data')
                .update({ status: 'วางบิลแล้ว', invoice_no: res.invoiceNo })
            ...
            let insertPayload = tempPayloadForPDF.map(item => ({
                create_date: parseDate(item.date),
                cs: item.cs || null,
                container_type: item.type || null,
                mode: item.mode || null,
                customer: item.jobCustomer || null,
                load_place: item.load || null,
                booking: item.booking || null,
                cy_place: item.cy || null,
                cy_date: parseDate(item.cy_date),
                vgm: item.vgm || null,
                rtn_place: item.rtn || null,
                rtn_date: parseDate(item.rtn_date),
                closing_time: item.closing_time || null,
                agent: item.agent || null,
                comment: item.remark || null,
                status: 'วางบิลแล้ว',
                vehicle_plate: item.plate || null,
                price: Math.round(item.price) || 0,
                receive: item.adv1 || 0,
                return: item.adv2 || 0,
                extender: Math.round(item.ext1) || 0,
                drop_tail: Math.round(item.ext2) || 0,
                lose_time: Math.round(item.ext3) || 0,
                container_no: item.container || null,
                repair: String(item.adv4 || 0),
                cleaning: Math.round(item.adv5) || 0,
                terminal_charge: Math.round(item.adv6) || 0,
                total_income: Math.round(item.incTotal) || 0,
                total_advance: String(item.advTotal || 0),
                grand_total: Math.round(item.grandTotal) || 0,
                bill_status: 'วางบิลแล้ว',
                invoice_no: res.invoiceNo,
                ref_key: `${item.booking}_${item.container}_${res.invoiceNo}`,
            ...
            if (res.pdfUrls && res.pdfUrls.length > 1) {
                htmlContent = `<p class="text-muted">บิลแยกประเภทพร้อมแล้ว</p><div class="d-flex flex-column gap-3 mt-3 px-2"><a href="${res.pdfUrls[0]}" target="_blank" class="btn btn-primary rounded-pill fw-bold shadow-sm py-2">บิลค่าขนส่ง</a><a href="${res.pdfUrls[1]}" target="_blank" class="btn btn-light text-primary border-primary rounded-pill fw-bold shadow-sm py-2">บิลสำรองจ่าย</a></div>`;
            } else {
                htmlContent = `<p class="text-muted">เอกสารพร้อมใช้งาน</p><a href="${res.pdfUrl}" target="_blank" class="btn btn-primary btn-lg rounded-pill mt-2 px-5 fw-bold shadow-sm" onclick="Swal.close()"><i class="bi bi-file-earmark-pdf-fill me-2"></i> เปิดดู Invoice</a>`;
            }
            
            Swal.fire({
                icon: 'success', title: 'สร้าง Invoice สำเร็จ!', html: htmlContent, showConfirmButton: false, showCloseButton: true,
                didClose: () => { document.getElementById('tab-history').click(); forceSyncAll(); }
            });
            
        } else {
            Swal.fire({ icon: 'error', title: 'เกิดข้อผิดพลาดจากระบบออกเอกสาร', text: res.message });
        }
    } catch (err) { 
        Swal.fire('เกิดข้อผิดพลาด', err.message, 'error'); 
    }
...
async function loadHistory(forceSync = false) {
    ...
    try {
        const { data, error } = await supabaseClient.from('invoice_data').select('*').order('invoice_no', { ascending: false });
        if (error) throw error;

        let grouped = {};
        (data || []).forEach(r => {
            let invNo = r.invoice_no;
            if (!invNo) return;
            if (!grouped[invNo]) {
                grouped[invNo] = {
                    invoiceNo: invNo, 
                    cs: r.cs || '-', 
                    customer: r.customer || '-',
                    date: r.bill_date || r.create_date || r.created_at, 
                    totalAmount: 0, 
                    count: 0,
                    billingUser: r.user_action || 'Admin', 
                    receiptNo: r.receive_no || '', 
                    voucherNo: r.voucher_no || ''
                };
            }
            grouped[invNo].totalAmount += (parseFloat(r.grand_total) || 0);
            grouped[invNo].count++;
        });
...
function renderHistoryTable() {
    ...
    if (filteredHistoryData.length === 0) {
        ...
        dataToRender.forEach((r) => {
            ...
            let isOwnerOrAdmin = (currentUser === 'Admin' || true);
            let docStatusStr = ''; let menuItems = '';
            ...
            if (r.receiptNo) {
                ...
                menuItems += `<li><button class="dropdown-item text-success" type="button" onclick="printPDF('${r.receiptNo}')"><i class="bi bi-receipt me-2"></i>ใบเสร็จรับเงิน</button></li>`;
            }
            if (r.voucherNo) {
                docStatusStr += '<span class="badge bg-info bg-opacity-10 text-info border border-info mt-1">ใบสำคัญ</span>';
                menuItems += `<li><button class="dropdown-item text-info" type="button" onclick="printPDF('${r.voucherNo}')"><i class="bi bi-cash-coin me-2"></i>ใบสำคัญรับ</button></li>`;
            }
            
            if (!r.receiptNo || !r.voucherNo) {
                menuItems += `<li><hr class="dropdown-divider"></li>`;
                if (!r.receiptNo) menuItems += `<li><button class="dropdown-item text-success" type="button" onclick="openReceiptModal('${r.invoiceNo}', 'RECEIPT')"><i class="bi bi-plus-circle me-2"></i>สร้างใบเสร็จรับเงิน</button></li>`;
                if (!r.voucherNo) menuItems += `<li><button class="dropdown-item text-info" type="button" onclick="openReceiptModal('${r.invoiceNo}', 'VOUCHER')"><i class="bi bi-plus-circle me-2"></i>สร้างใบสำคัญรับ</button></li>`;
            }

            menuItems += `<li><hr class="dropdown-divider"></li>`;
            if (isOwnerOrAdmin) {
                if (r.receiptNo) menuItems += `<li><button class="dropdown-item text-danger" type="button" onclick="promptRollbackReceipt('${r.invoiceNo}', '${r.receiptNo}', 'REC')"><i class="bi bi-trash me-2"></i>ลบใบเสร็จรับเงิน</button></li>`;
                if (r.voucherNo) menuItems += `<li><button class="dropdown-item text-danger" type="button" onclick="promptRollbackReceipt('${r.invoiceNo}', '${r.voucherNo}', 'VOU')"><i class="bi bi-trash me-2"></i>ลบใบสำคัญรับ</button></li>`;
                if (!r.receiptNo && !r.voucherNo) {
                    ...
                    menuItems += `<li><button class="dropdown-item text-muted" type="button" onclick="Swal.fire({icon:'warning', text:'กรุณาลบใบเสร็จ/ใบสำคัญรับ ทิ้งก่อน จึงจะแก้ไข/ยกเลิกบิลได้'})"> <i class="bi bi-pencil-square me-2"></i>แก้ไขบิลโดยตรง </button></li>`;
                    menuItems += `<li><button class="dropdown-item text-muted" type="button" onclick="Swal.fire({icon:'warning', text:'กรุณาลบใบเสร็จ/ใบสำคัญรับ ทิ้งก่อน จึงจะแก้ไข/ยกเลิกบิลได้'})"> <i class="bi bi-arrow-counterclockwise me-2"></i>ยกเลิกบิล </button></li>`;
                }
            }
            ...
            html += `
                <tr>
                    ...
                    <td class="text-center"><span class="badge bg-light text-dark border rounded-pill px-3 shadow-sm">${r.count}</span></td>
                    <td class="text-end fw-bold text-primary fs-6">฿${r.totalAmount.toLocaleString(undefined, {minimumFractionDigits:2})}</td>
                    <td class="text-center pe-4">
                        <div class="btn-group dropstart">
                            <button type="button" class="btn btn-white border shadow-sm btn-sm fw-bold dropdown-toggle dropdown-toggle-split rounded-pill px-3 text-secondary" data-bs-toggle="dropdown"><span class="visually-hidden">เปิดเมนู</span><i class="bi bi-three-dots"></i></button>
...
async function viewInvoiceDetails(invNo) {
    try {
        ...
        let html = `<div class="bg-white border rounded-4 overflow-hidden mt-3 shadow-sm"><table class="table table-borderless table-hover text-nowrap m-0" style="font-size: 0.82rem;"><thead class="bg-light text-muted border-bottom"><tr><th class="py-3 px-3">ตู้คอนเทนเนอร์</th><th>ทะเบียนรถ</th><th class="text-end">รายได้</th><th class="text-end">สำรองจ่าย</th><th class="text-end pe-3">รวมสุทธิ</th></tr></thead><tbody>`;
        data.forEach(r => {
            let inc = parseFloat(r.total_income) || 0;
            let adv = parseFloat(r.total_advance) || 0;
            html += `<tr><td class="text-start fw-bold text-primary py-3 px-3">${r.container_no||'-'}</td><td class="text-start text-secondary">${r.vehicle_plate||'-'}</td><td class="text-end text-success fw-medium">฿${inc.toLocaleString(undefined, {minimumFractionDigits:2})}</td><td class="text-end text-danger fw-medium">฿${adv.toLocaleString(undefined, {minimumFractionDigits:2})}</td><td class="text-end fw-bold text-dark pe-3">฿${(inc+adv).toLocaleString(undefined, {minimumFractionDigits:2})}</td></tr>`;
        });
        html += `</tbody></table></div>`;
        
        Swal.fire({ title: `<span class="fw-bold fs-4 text-dark">${invNo}</span>`, html: `<div class="table-responsive" style="max-height: 400px; padding:0;">${html}</div>`, width: '650px', showCloseButton: true, showConfirmButton: false });
...
async function openEditInvoiceModal(invoiceNo) {
    try {
        ...
        document.getElementById('editInvNo').value = invoiceNo;
        document.getElementById('editInvDate').value = data[0].bill_date || data[0].create_date || '';
        document.getElementById('editCustomer').value = data[0].customer || '';
        document.getElementById('editIsSplit').checked = false;

        let html = '';
        data.forEach((itm) => {
            html += `
            <div class="card shadow-sm border border-light mb-3 edit-item-card rounded-3 overflow-hidden" data-booking="${itm.booking}" data-container="${itm.container_no}" data-rowid="${itm.id}">
                <div class="card-header bg-white border-bottom pt-2 pb-1">
                    <h6 class="fw-bold text-dark mb-0 fs-6"><i class="bi bi-box-seam me-2 text-primary"></i>${itm.container_no} <span class="badge bg-light text-secondary border fw-medium ms-2">BKG: ${itm.booking}</span></h6>
                </div>
                <div class="card-body bg-light p-2 px-3">
                    <div class="row g-3">
                        <div class="col-md-6 border-end border-light">
                            ...
                            <div class="row g-2">
                                <div class="col-6"><label class="small text-muted mb-0" style="font-size: 0.75rem;">ราคาเที่ยว</label><input type="number" class="form-control form-control-sm bg-white e-price" value="${itm.price || 0}"></div>
                                <div class="col-6"><label class="small text-muted mb-0" style="font-size: 0.75rem;">ต่อระยะ</label><input type="number" class="form-control form-control-sm bg-white e-ext1" value="${itm.extender || 0}"></div>
                                <div class="col-6"><label class="small text-muted mb-0" style="font-size: 0.75rem;">ค้างหาง</label><input type="number" class="form-control form-control-sm bg-white e-ext2" value="${itm.drop_tail || 0}"></div>
                                <div class="col-6"><label class="small text-muted mb-0" style="font-size: 0.75rem;">เสียเวลา</label><input type="number" class="form-control form-control-sm bg-white e-ext3" value="${itm.lose_time || 0}"></div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <p class="text-danger fw-bold small mb-2"><i class="bi bi-arrow-down-right-circle me-1"></i>สำรองจ่าย (Advance)</p>
                            <div class="row g-2">
                                <div class="col-4"><label class="small text-muted mb-0" style="font-size: 0.75rem;">รับตู้</label><input type="number" class="form-control form-control-sm bg-white e-adv1" value="${itm.receive || 0}"></div>
                                <div class="col-4"><label class="small text-muted mb-0" style="font-size: 0.75rem;">คืนตู้</label><input type="number" class="form-control form-control-sm bg-white e-adv2" value="${itm.return || 0}"></div>
                                <div class="col-4"><label class="small text-muted mb-0" style="font-size: 0.75rem;">ผ่านท่า</label><input type="number" class="form-control form-control-sm bg-white e-adv3" value="${itm.terminal_charge || 0}"></div>
                                <div class="col-4"><label class="small text-muted mb-0" style="font-size: 0.75rem;">ซ่อมตู้</label><input type="number" class="form-control form-control-sm bg-white e-adv4" value="${itm.repair || 0}"></div>
                                <div class="col-4"><label class="small text-muted mb-0" style="font-size: 0.75rem;">ล้างตู้</label><input type="number" class="form-control form-control-sm bg-white e-adv5" value="${itm.cleaning || 0}"></div>
                                <div class="col-4"><label class="small text-primary fw-bold mb-0" style="font-size: 0.75rem;">ค่าชอ</label><input type="number" class="form-control form-control-sm border-primary bg-primary bg-opacity-10 e-adv6" value="${itm.shore || 0}"></div>
                                <div class="col-12 m-0 mt-1"><hr class="m-0 opacity-25"></div>
                                <div class="col-8 mt-1"><label class="small text-muted mb-0" style="font-size: 0.75rem;">ยอดอื่น 1 (ชื่อ)</label><input type="text" class="form-control form-control-sm bg-white e-extn1" value="${itm.other_exp_name_1 || ''}"></div>
                                <div class="col-4 mt-1"><label class="small text-muted mb-0" style="font-size: 0.75rem;">ยอด</label><input type="number" class="form-control form-control-sm bg-white e-extv1" value="${itm.other_exp_amt_1 || 0}"></div>
                            </div>
                        </div>
                    </div>
                </div>
...
async function confirmEditInvoice() {
    ...
    Swal.fire({
        title: 'ยืนยันการอัปเดต?', text: "ระบบจะสร้าง PDF ใบใหม่ทับของเดิม", icon: 'warning',
        showCancelButton: true, confirmButtonColor: '#1e293b', confirmButtonText: 'ยืนยันอัปเดต', cancelButtonText: 'ยกเลิก'
    }).then(async (result) => {
        if (result.isConfirmed) {
            ...
            try {
                ...
                let custInfoToSend = {
                    officialName: custData ? custData.full_name : customer,
                    address: custData && custData.address ? custData.address : "",
                    taxId: custData && custData.tax_id ? String(custData.tax_id) : "",
                    creditDays: custData && custData.credit_days ? parseInt(custData.credit_days) : 0
                };
                ...
                cards.forEach(card => {
                    ...
                    let container = card.getAttribute('data-container');
                    let oldItem = oldData.find(o => o.booking === booking && o.container_no === container) || {};

                    let p = parseFloat(card.querySelector('.e-price').value) || 0;
                    let e1 = parseFloat(card.querySelector('.e-ext1').value) || 0;
                    let e2 = parseFloat(card.querySelector('.e-ext2').value) || 0;
                    let e3 = parseFloat(card.querySelector('.e-ext3').value) || 0;
                    let a1 = parseFloat(card.querySelector('.e-adv1').value) || 0;
                    let a2 = parseFloat(card.querySelector('.e-adv2').value) || 0;
                    let a3 = parseFloat(card.querySelector('.e-adv3').value) || 0;
                    let a4 = parseFloat(card.querySelector('.e-adv4').value) || 0;
                    let a5 = parseFloat(card.querySelector('.e-adv5').value) || 0;
                    let a6 = parseFloat(card.querySelector('.e-adv6').value) || 0;
                    let n1 = card.querySelector('.e-extn1').value.trim();
                    let v1 = parseFloat(card.querySelector('.e-extv1').value) || 0;

                    let incTot = p + e1 + e2 + e3;
                    let advTot = a1 + a2 + a3 + a4 + a5 + a6 + v1;

                    payload.push({
                        rowIdx: oldItem.id, 
                        jobCustomer: oldItem.customer, customer: customer, booking: booking, date: oldItem.create_date, plate: oldItem.vehicle_plate || '-',
                        cy: oldItem.cy_place || '-', load: oldItem.load_place || '-', rtn: oldItem.rtn_place || '-', type: oldItem.container_type || '-', container: container, remark: oldItem.comment || '',
                    ...
                    insertPayload.push({
                        ...
                        customer: customer,
                        bill_date: invDate,
                        price: p, extender: e1, drop_tail: e2, lose_time: e3,
                        receive: a1, return: a2, terminal_charge: a3, repair: String(a4), cleaning: a5, shore: a6,
                        other_exp_name_1: n1, other_exp_amt_1: v1,
                        total_income: incTot, total_advance: String(advTot), grand_total: incTot + advTot,
                        user_action: currentUser
                    });
                });
                ...
                const res = await callInvAPI('generateInvoicePDF', { 
                    payload: payload, invoiceDate: invDate, billingUser: currentUser, isSplit: isSplit, customInvNo: invNo, custInfo: custInfoToSend 
                }, 1, false);
                
                if (res.success) {
                    await supabaseClient.from('invoice_data').delete().eq('invoice_no', invNo);
                    await supabaseClient.from('invoice_data').insert(insertPayload);
                    ...
                    let htmlContent = '';
                    if (res.pdfUrls && res.pdfUrls.length > 1) {
                        htmlContent = `<p class="text-muted">เอกสารใหม่พร้อมแล้ว</p><div class="d-flex flex-column gap-3 mt-3"><a href="${res.pdfUrls[0]}" target="_blank" class="btn btn-primary rounded-pill fw-bold">บิลค่าขนส่ง</a><a href="${res.pdfUrls[1]}" target="_blank" class="btn btn-light text-primary border-primary rounded-pill fw-bold">บิลสำรองจ่าย</a></div>`;
                    } else {
                        htmlContent = `<p class="text-muted">อัปเดตเอกสารเรียบร้อยแล้ว</p><a href="${res.pdfUrl}" target="_blank" class="btn btn-dark btn-lg rounded-pill mt-3 px-5 fw-bold shadow-sm" onclick="Swal.close()"><i class="bi bi-file-earmark-pdf me-2"></i> เปิดดูเอกสารที่อัปเดต</a>`;
                    }
...
function promptRollback(invoiceNo) {
    Swal.fire({
        title: 'ยกเลิกบิล?', html: `ต้องการยกเลิก Invoice <b class="text-danger">${invoiceNo}</b> และส่งกลับไปหน้าเตรียมวางบิลหรือไม่`, icon: 'warning',
        showCancelButton: true, confirmButtonColor: '#ef4444', confirmButtonText: 'ยืนยันยกเลิกบิล', cancelButtonText: 'ปิด'
    }).then(async res => {
        if (res.isConfirmed) {
...
function promptRollbackReceipt(invoiceNo, docNoToDel, docTypeToDel) {
    ...
    Swal.fire({
        title: `ลบ${txt}?`, html: `คุณแน่ใจหรือไม่ว่าต้องการลบ <b class="text-danger">${docNoToDel}</b>`, icon: 'warning',
        showCancelButton: true, confirmButtonColor: '#ef4444', confirmButtonText: `ยืนยันลบ`, cancelButtonText: 'ยกเลิก'
    }).then(async res => {
        if (res.isConfirmed) {
...
async function printPDF(docNo) {
    try {
        ...
        if (res.success) {
            Swal.fire({ title: `<i class="bi bi-file-earmark-pdf text-primary fs-1"></i>`, html: `<h4 class="fw-bold mb-3">${docNo}</h4><a href="${res.url}" target="_blank" class="btn btn-primary btn-lg rounded-pill px-5 fw-bold shadow" onclick="Swal.close()">เปิดเอกสาร</a>`, showConfirmButton: false, showCloseButton: true });
...
function openReceiptModal(invNo, docType) {
    ...
    let today = new Date();
    if (document.getElementById('modReceiptDate')) document.getElementById('modReceiptDate').value = today.getFullYear() + "-" + String(today.getMonth() + 1).padStart(2, '0') + "-" + String(today.getDate()).padStart(2, '0');
...
async function confirmGenerateReceipt() {
    ...
    try {
        ...
        let custInfoToSend = {
            officialName: cData ? cData.full_name : bData[0].customer,
            address: cData && cData.address ? cData.address : "",
            taxId: cData && cData.tax_id ? String(cData.tax_id) : "",
            creditDays: cData && cData.credit_days ? parseInt(cData.credit_days) : 0
        };
        ...
        let itemsPayload = bData.map(r => {
            ...
            let adv = parseFloat(r.total_advance) || 0;
            return {
                container: r.container_no, plate: r.vehicle_plate, amountToBill: currentDocType === 'VOUCHER' ? adv : inc,
                advDetails: { 
                    a1: parseFloat(r.receive)||0, a2: parseFloat(r.return)||0, a_pass: parseFloat(r.terminal_charge)||0, 
                    a_repair: parseFloat(r.repair)||0, a_clean: parseFloat(r.cleaning)||0, a_cho: parseFloat(r.shore)||0, n1: r.other_exp_name_1||'', v1: parseFloat(r.other_exp_amt_1)||0 
                }
            }
        });
        ...
        if (res.success) {
            ...
            if (error) {
                ...
                Swal.fire('บันทึกสถานะไม่สำเร็จ', `ออกเอกสารสำเร็จแต่บันทึกสถานะไม่ได้ กรุณาตรวจสอบว่ามีคอลัมน์ receive_no และ voucher_no ในตาราง invoice_data หรือไม่\n\nรายละเอียด: ${error.message}`, 'error');
                return;
            }
            ...
            Swal.fire({
                icon: 'success', title: `สร้าง${docName}สำเร็จ!`, html: `<a href="${res.pdfUrl}" target="_blank" class="btn ${btnClass} btn-lg rounded-pill mt-3 px-5 fw-bold shadow-sm" onclick="Swal.close()"><i class="bi bi-file-earmark-pdf-fill me-2"></i> เปิดดูเอกสาร</a>`, showConfirmButton: false, showCloseButton: true,
                didClose: () => { forceSyncAll(); }
            });
        } else { 
            if (typeof hideGlobalLoader === 'function') hideGlobalLoader();
            document.getElementById('btnConfirmReceipt').disabled = false;
