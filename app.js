...
function startBackgroundSync() {
    ...
    realtimeChannel = supabaseClient
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'plan_data' }, async (payload) => {
            console.log('🔄 อัปเดตข้อมูล Realtime:', payload);
...
async function loadPlanDataInitial() {
    ...
    try {
        ...
        if (document.getElementById('csDataList')) {
            document.getElementById('csDataList').innerHTML = uniqueCS.map(cs => `<option value="${cs}">`).join('');
        }
...
function applyFilters() {
    ...
    const sText = getVal('filterText'); const fCS = getVal('filterCS'); const fMode = getVal('filterMode');
    const fStatus = getValExact('filterStatus'); const fYear = getValExact('filterYear');
    const fMonth = getValExact('filterMonth'); const fDay = getValExact('filterDay');
    ...
    filteredData = allData.filter((row, index) => {
        ...
        return matchText && matchDate && matchCS && matchMode && matchStatus;
    });
...
function clearFilters() { 
    ...
    resetVal('filterText'); resetVal('filterCS'); resetVal('filterMode'); resetVal('filterStatus'); 
    resetVal('filterYear'); resetVal('filterMonth'); resetVal('filterDay'); 
...
function handleCustomerSelect() {
    autoFillBillingNamesAdd();
}

function mapToFullName(inputElem) {
    let val = inputElem.value.trim();
    if(val && customerMapGlobal[val]) inputElem.value = customerMapGlobal[val];
}
    ...
    <div class="card mb-3 border-${col} shadow-sm container-row fade-in">
        ...
        <div class="card-body p-3 compact-form">
            <div class="row g-2 align-items-end mb-2">
                <div class="col-6 col-md-3"><label class="fw-bold small text-muted mb-1">CY Place</label><input type="text" list="cyPlaceList" class="form-control form-control-sm cyPlace" value="${defCyPlace}"></div>
                ...
                <div class="col-6 col-md-3"><label class="fw-bold small text-muted mb-1">RTN Place</label><input type="text" list="rtnPlaceList" class="form-control form-control-sm rtnPlace" value="${defRtnPlace}"></div>
            ...
            <div class="row g-2 align-items-center pt-2 border-top mt-2">
                <div class="col-6 col-md-2"><label class="fw-bold text-danger small mb-1">ราคา/ตู้</label><input type="number" class="form-control form-control-sm border-danger fw-bold text-success cPrice" value="${mPrice}"></div>
...
async function saveData() {
  ...
  rows.forEach(r => {
    ...
    payloadData.push({
        ...
        cs: currentUser,
        container_type: r.querySelector('.cType').value,
        mode: document.getElementById('mode').value,
        customer: customer,
        load_place: loadPlace,
        booking: bkg,
        cy_place: r.querySelector('.cyPlace').value,
        cy_date: r.querySelector('.cyDate').value || null,
        vgm: document.getElementById('vgm').value,
        rtn_place: r.querySelector('.rtnPlace').value,
        rtn_date: r.querySelector('.rtnDate').value || null,
        closing_time: document.getElementById('closing').value || null,
        agent: document.getElementById('agent').value,
        comment: r.querySelector('.cComment').value,
        status: isChecked ? 'กำลังไปรับตู้' : 'รอจัดรถ',
        vehicle_plate: plate,
        price: parseInt(r.querySelector('.cPrice').value) || 0,
        receive: (r.querySelector('.cExp1').value || 0).toString(),
        retrun: (r.querySelector('.cExp2').value || 0).toString(),
        extender: parseInt(r.querySelector('.cExp3').value) || 0,
        tail_drop: parseInt(r.querySelector('.cExp4').value) || 0,
        lose_time: parseInt(r.querySelector('.cExp5').value) || 0,
        container_no: container,
        terminal_charge: parseInt(r.querySelector('.cExp6').value) || 0,
        repair: (r.querySelector('.cExp7').value || 0).toString(),
        cleaning: parseInt(r.querySelector('.cExp8').value) || 0,
        shore: parseInt(r.querySelector('.cExp9').value) || 0,
        other_exp_name_1: r.querySelector('.cExp10Name').value.trim(),
        other_exp_amt_1: parseInt(r.querySelector('.cExp10Val').value) || 0,
        other_exp_name_2: r.querySelector('.cExp11Name').value.trim(),
        other_exp_amt_2: parseInt(r.querySelector('.cExp11Val').value) || 0,
        invoice_no: "",
        history_edit: "",
        bill_to_name: billToVal,
        ชื่อออกใบเสร็จ: receiptVal
    });
  });

  if(hasError) return Swal.fire({ icon: 'warning', title: 'ข้อมูลไม่ครบ', text: 'กรุณาระบุ "เบอร์ตู้" และ "ทะเบียนรถ" ให้ครบถ้วนสำหรับตู้ที่ติ๊กจัดรถแล้ว' });
...
function openEditModal(rowNum) {
  ...
  if(currentStatus === 'พร้อมวางบิล' || currentStatus === 'วางบิลแล้ว') {
      btnSave.disabled = true; btnSave.innerHTML = `<i class="bi bi-lock-fill"></i> ${currentStatus} (แก้ไขไม่ได้)`; btnSave.classList.replace('btn-success', 'btn-secondary');
      ...
      btnSave.disabled = false; btnSave.innerHTML = '<i class="bi bi-floppy-fill"></i> บันทึกแก้ไข'; btnSave.classList.replace('btn-secondary', 'btn-success');
      btnFinishBooking.disabled = false; btnDeleteSingle.disabled = false;
      inputs.forEach(inp => { if(inp.id !== 'eCS' && inp.id !== 'eInvoiceNo') { inp.disabled = false; inp.classList.remove('bg-light'); } });
  }
...
async function saveEdit() {
  ...
  if(status !== 'รอจัดรถ' && (!plate || !container)) return Swal.fire({ icon: 'warning', title: 'ข้อมูลไม่ครบ', text: 'หากสถานะมีการจัดรถแล้ว ต้องระบุ เบอร์ตู้ และ ทะเบียนรถ เสมอ' });
...
async function finishEntireBooking() {
    ...
    if(!currentPlate || !currentContainer) return Swal.fire({ icon: 'warning', title: 'ข้อมูลไม่ครบ', text: 'กรุณาระบุ เบอร์ตู้ และ ทะเบียนรถ ในตู้นี้ให้ครบก่อนครับ' });
    ...
    for(let r of otherRows) {
        ...
        if(!r[16] || !r[23]) return Swal.fire({icon: 'warning', title: 'ข้อมูลไม่ครบ', text: `ตู้ใบอื่นใน Booking นี้ยังไม่ได้ระบุ เบอร์ตู้ หรือ ทะเบียนรถ\nกรุณาใส่ให้ครบทุกใบก่อนกดจบงานรวดเดียวครับ`});
    }

    let mappedName = customerMapGlobal[v('eCustomer').trim()] || v('eCustomer').trim();

    Swal.fire({
        title: 'ยืนยันจบงานรวดเดียว?', html: `ระบบจะเซฟข้อมูลตู้ใบนี้และเปลี่ยนสถานะตู้ทั้งหมดใน<br><b class="text-primary">${bkgNo}</b> เป็น <b class="text-success">"จบงานรอวางบิล"</b> หรือไม่?`, icon: 'question',
        ...
        if(res.isConfirmed) {
            ...
            if (idsToUpdate.length > 0) {
                await supabaseClient.from('plan_data').update({ status: 'จบงานรอวางบิล' }).in('id', idsToUpdate);
            }

            await writeLog("จบงาน (Batch)", `บันทึกและเปลี่ยนสถานะเป็นจบงานรอวางบิล BKG: ${bkgNo}`);
            bootstrap.Modal.getInstance(document.getElementById('editModal')).hide(); await loadPlanData(true);
            hideGlobalLoader();
...
function deleteSingle() {
  ...
  if(status === 'วางบิลแล้ว' || status === 'พร้อมวางบิล') return Swal.fire({ icon: 'error', title: 'ลบไม่ได้!', text: `รายการนี้อยู่ในสถานะ "${status}" ไปแล้ว ไม่สามารถลบได้ครับ` });
...
function renderBatchTruckTable() {
  ...
  if (unassignedRows.length === 0) { html = `<tr><td colspan="7" class="text-center py-4 text-muted">ไม่พบตู้ที่รอจัดรถ</td></tr>`; } 
  else {
...
async function saveBatchTruckMulti() {
  ...
  if (hasError) return Swal.fire({ icon: 'warning', title: 'ข้อมูลไม่ครบ!', text: 'กรุณาระบุ ทะเบียนรถ และ เบอร์ตู้ ให้ครบถ้วนสำหรับงานที่ต้องการจัดรถ' });
